...
static struct bt_gatt_attr service_b_3_2_attrs[] = {
	BT_GATT_PRIMARY_SERVICE(BT_UUID_SERVICE_B_3, 0x70),
	...
	BT_GATT_CCC(value_v6_ccc_cfg, value_v6_ccc_cfg_changed, 0x73)
};
 ...
 * @param err  The error code from the indicate attempt, 0 if no error
 *             or BT_GATT_ERR() with a specific ATT error code.
 */
...
void service_b_3_2_value_v6_indicate(void)
{
	if (!value_v6_ind_active)
		return;
	/*
	 * NOTE: Zephyr doesn't automatically bump up the attribute pointer
	 * for indications as it does for notifications.
	 */
