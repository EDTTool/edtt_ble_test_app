...
/**
 * @brief Provide an error reponse when an HCI command send failed...
 */
...
#endif
NET_BUF_POOL_DEFINE(hci_cmd_pool, CONFIG_BT_HCI_CMD_COUNT, BT_BUF_RX_SIZE, BT_BUF_USER_DATA_MIN, NULL);
NET_BUF_POOL_DEFINE(hci_data_pool, CONFIG_BT_CTLR_TX_BUFFERS+4, BT_BUF_ACL_SIZE, BT_BUF_USER_DATA_MIN, NULL);
...
static int send_hci_command(u16_t opcode, u8_t param_len, u16_t response)
{
    ...
    if ((buf = hci_cmd_create(waiting_opcode = opcode, param_len))) {
        if (param_len) {
...
static void service_events(void *p1, void *p2, void *p3)
{
    ...
    while (1) {
        ...
        else if (BT_BUF_ACL_IN == bt_buf_get_type(buf)) {
            struct net_buf *data;

            if ((data = net_buf_alloc(&data_pool, K_NO_WAIT))) {
...
static void get_event(u16_t size)
{
    ...
    edtt_write((u8_t *)&response, sizeof(response), EDTTT_BLOCK);
    if ((buf = net_buf_get(&event_queue, K_FOREVER))) {
...
static void le_data_ready(u16_t size)
{
    ...
    struct has_data_resp le_response = { sys_cpu_to_le16(CMD_LE_DATA_READY_RSP), sys_cpu_to_le16(1), 0 };
...
static void le_data_read(u16_t size)
{
    ...
    edtt_write((u8_t *)&response, sizeof(response), EDTTT_BLOCK);
    if ((buf = net_buf_get(&data_queue, K_FOREVER))) {
        size = sys_cpu_to_le16(buf->len);
        edtt_write((u8_t *)&size, sizeof(size), EDTTT_BLOCK);
        edtt_write((u8_t *)buf->data, buf->len, EDTTT_BLOCK);
        net_buf_unref(buf);
...
static void le_data_write(u16_t size)
{
    ...
    struct data_write_resp response = { sys_cpu_to_le16(CMD_LE_DATA_WRITE_RSP), sys_cpu_to_le16(1), 0 };
    struct net_buf *buf;
    struct bt_hci_acl_hdr hdr;
    int    err;
    ...
    if (size >= sizeof(hdr)) {
        ...
        err = -3; /* Size too small for header (handle and data length) */
    }
    read_excess_bytes(size);
...
void main(void)
{
    ...
    u16_t opcode;
    /**
     * Initialize HCI command opcode and reponse variables...
     */
    waiting_opcode = 0;
    waiting_response = CMD_NOTHING;
    m_events = 0;
    ...
    while (1) {
        ...
        switch (command) {
            ...
            default:
                 if (size >= 2) {
                     edtt_read((u8_t *)&opcode, sizeof(opcode), EDTTT_BLOCK);
                     send_hci_command(sys_le16_to_cpu(opcode), size-2, command+1);
                 }
        }
    }
}
